...
### Value Border
Border bisa diterapkan pada 4 sisi:
1. **Top**  
2. **Right**  
3. **Bottom**  
4. **Left**
...
**Padding** digunakan untuk memberikan ruang kosong di dalam elemen, yaitu jarak antara content dengan border.  
...
**Float** digunakan untuk membuat elemen “melayang” ke kiri atau kanan dari kontainernya,  
memungkinkan teks atau elemen lain mengalir di sekitarnya.  
Biasanya dipakai untuk layout gambar di samping teks, atau membuat kolom sederhana sebelum era Flexbox dan Grid.

### Value Float
Float memiliki 3 value utama:
...
### Contoh HTML
```html
<img src="gambar.jpg" style="float: left; margin: 10px;">
<p>
  Teks ini akan mengalir di sisi kanan gambar.
</p>

## Property: Position

**Position** digunakan untuk menentukan cara sebuah elemen ditempatkan dalam halaman web.  
...
**Box-Sizing** digunakan untuk menentukan cara browser menghitung ukuran total elemen (`width` dan `height`).  
...
Salah satu reset yang paling terkenal adalah **Eric Meyer’s Reset CSS**.

### Contoh CSS Reset (Eric Meyer)
```css
...
**Clearfix** adalah teknik CSS yang digunakan untuk mengatasi masalah layout akibat elemen `float`.  
Tanpa clearfix, elemen pembungkus sering kali tidak menghitung tinggi anak yang di-*float*, sehingga layout bisa rusak.

---

### 1. Clearfix Dasar (Douglas / Thierry Koblentz)
Teknik ini menggunakan pseudo-element `::after` untuk menambahkan blok kosong yang melakukan `clear: both`.
...
**Z-Index** digunakan untuk mengatur tumpukan atau lapisan sebuah elemen HTML.  
